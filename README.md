...
python -m pytest                        # 222 tests, no network, no credentials
ruff check . && ruff format --check .   # rule set pinned in pyproject.toml
mypy                                    # strict, src only
```

Nothing above touches a live instance.
...
- **Writes are retried on 429 and 503, and on nothing else — and replay is made harmless separately.** A read can be replayed freely. A `POST` that timed out may already have created the incident, and quietly creating two P1s is worse than surfacing the timeout, so timeouts and 500s fail the write loudly; `ConnectError` is the exception, because no bytes reached the instance. `Retry-After` is honoured as seconds or HTTP-date and then clamped, so a misconfigured gateway advertising an hour cannot park the server for an hour. Retrying less is only half the answer, though: agent retries and client reconnects replay tool calls whatever the transport does, so `create_incident` takes a `correlation_id`, and an incident already carrying it is returned instead of written again. Idempotency is a parameter, not a hope.
...
- **Not verified against a live instance.** The suite runs against an in-memory fake built to the documented Table API contract, which is what makes it run offline — but a fake agrees with its author, not with production. The two likeliest divergences are ACL evaluation, which silently removes rows and columns rather than erroring, and instances whose state or priority model has been customised away from the out-of-box values.
